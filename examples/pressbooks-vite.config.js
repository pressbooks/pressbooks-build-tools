...
export default createViteConfig( {
	// JavaScript entries - each .js() call becomes an input entry
	input: {
		// Core scripts
	...
	// File copying (replaces .copy() and .copyDirectory() from Mix)
	copyTargets: [
...
/*
 * NOTES ON MIGRATION FROM LARAVEL MIX:
 *
 * 1. .scripts() concatenation:
 *    For vendor scripts that were concatenated with .scripts(), you have options:
 *    a) Create wrapper JS files that import the modules (recommended)
 *    b) Use copyTargets to copy them individually
 *    c) Bundle them using Vite's manual chunks configuration
 *
 * 2. File versioning:
 *    Vite handles this automatically in production builds
 *
 * 3. BrowserSync:
 *    Replaced with Vite's built-in dev server + live reload plugin
 *
 * 4. CSS processing:
 *    Vite handles SCSS processing automatically, no need for processCssUrls option
 */
